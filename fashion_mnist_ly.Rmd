...
- 20 custom blocks that are subsections of the grids, showing percent non-white as well as percent light grey (1 - 122).
...
After calculating the prediction accuracy on the test set, the random forest model has a minor lead over the support vector machine model.

### Classification Evaluation

```{r eval rf}
# confusion matrix for random forest
with(test2, confusionMatrix(rf, label))
```

```{r eval svm}
# confusion matrix for SVM Radial
confusionMatrix(test2$svm, test2$label)
```

```{r eval knn}
# confusion matrix for SVM Radial
confusionMatrix(test2$knn, test2$label)
```

```{r eval multinomial}
# confusion matrix for multinomial regression
with(test2, confusionMatrix(mn, label))
```

```{r eval naive_bayes}
# confusion matrix for naive bayes
with(test2, confusionMatrix(nb, label))
```
...
The random forest model commonly misclassifies pullovers as coats and shirts as t-shirts/tops or pullovers. Additionally, for each of the three types of footwear, the errors predicted by the model are most likely one of other two footwear types.
