...
Using K = 2, we created an almost even split of observations.

```{r cluster viz adhd}
# adhd vs suicide
fviz_cluster(cluster_k2, data = df4, c('adhd_total', 'suicide'))
```
The clusters created show a clear delineation for respondents based on `adhd_total`.
...
The results of the k2 cluster show considerable overlap based on `md_total`.

### Relative Cluster Differences

Let's show relative distributions for each cluster and variable.
